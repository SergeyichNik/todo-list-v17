...
const slice = createSlice({
    ...
    reducers: {
        removeTaskAC(state, action: PayloadAction<{ taskId: string, todolistId: string }>) {
            ...
            if (taskIndex > -1) state[todolistId].splice(taskIndex, 1)
        },
        ...
        updateTaskAC(state, action: PayloadAction<{ taskId: string, model: UpdateDomainTaskModelType, todolistId: string }>) {
            ...
            const taskIndex = state[todolistId].findIndex(task => task.id === taskId);
            if (taskIndex > -1) {
                state[todolistId][taskIndex] = {...state[todolistId][taskIndex], ...model};
            }
        },
...
export const updateTaskTC = (taskId: string, domainModel: UpdateDomainTaskModelType, todolistId: string) =>
    (dispatch: ThunkDispatch, getState: () => AppRootStateType) => {
        ...
        todolistsAPI.updateTask(todolistId, taskId, apiModel)
            .then(res => {
                if (res.data.resultCode === 0) {
                    const action = updateTaskAC({taskId, model: apiModel, todolistId})
                    dispatch(action)
                } else {
                    handleServerAppError(res.data, dispatch);
                }
